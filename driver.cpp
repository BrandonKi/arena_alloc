...
int main() {
	arena_allocator<int> alloc;
	std::vector<int, arena_allocator<int>> vec1(100, alloc);
	std::iota(vec1.begin(), vec1.end(), 0);
	std::for_each(vec1.cbegin(), vec1.cend(), [](int i) { std::cout << i << ','; });

	std::vector<int, arena_allocator<int>> vec2(100, alloc);
	std::iota(vec2.begin(), vec2.end(), 0);
	std::for_each(vec2.cbegin(), vec2.cend(), [](int i) { std::cout << i << ','; });

	std::vector<int, arena_allocator<int>> vec3(200, alloc);
